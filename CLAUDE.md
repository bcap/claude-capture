...
- Run claude with capture: `./claude.sh [args forwarded to claude]`
...
- The addons are resolved by `claude.sh` via `$DIR="$(dirname "$0")"`, so they must stay co-located with the wrapper script.
...
- `claude.sh` is meant to be a drop-in: don't add prompts, change exit-code semantics, or reorder so post-processing depends on claude's success — the capture must be flushed and compressed even on non-zero/crash exits.
